		...
		const char *buf, size_t size)				
{
	...
	if (sscanf(buf, "%08x\n", &value) == 1) {			
		...
		if(value == 0x4948	) // debug level high
			log_usb = true;
		else
			log_usb = false;

		printk(KERN_DEBUG "usb: %s log_usb %d  value %x\n", __func__, log_usb,value );
		return size;
	}		
...
static DEVICE_ATTR(iDebugLevel, S_IRUGO | S_IWUSR, NULL, debug_store);
/*  */
#endif
